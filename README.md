# 网络监控 - Network Monitor

> 🇨🇳 中文 | [🇬🇧 English](README_EN.md)

BitTorrent Tracker 全功能监控工具，支持 TCP/UDP Ping、多IP同时监控、IPv4+IPv6 双栈、可用率排行、SOCKS5 代理、多账户权限管理。

---

## ✨ 功能特性

### 核心监控
- ✅ TCPing 检测（HTTP/HTTPS Tracker）
- ✅ UDPing 检测（UDP Tracker BEP 15 协议握手）
- ✅ 同一域名多IP监控，展开查看每个IP详情
- ✅ IPv4 + IPv6 全解析，同时监控双栈
- ✅ 支持纯 `IP:端口` 模式直接添加
- ✅ 并发检测线程数可配置（默认120，建议30～200）
- ✅ 失败自动重试（轮询模式：5s → 15s → 30s → 60s 递增间隔）
- ✅ 发包错峰：分批提交检测任务，避免瞬间高并发冲击本地网络或代理（批次大小、批间延迟可独立配置代理/直连两套参数）

### 可用率统计
- ✅ 24小时 / 7天 / 30天 可用率，按真实时间窗口计算（不依赖固定轮次计数）
- ✅ 监控列表与排行榜统计周期独立配置
- ✅ 历史数据持久化到 `history.json`（时间戳格式，重启不丢失，GC 自动清理30天前记录）
- ✅ `history.json` 原子写入保护：先写临时文件再替换，写入中断或崩溃不会损坏现有数据
- ✅ 启动时自动过滤无效 IP 记录（`[::]`、`127.0.0.1` 等 DNS 劫持结果），不污染历史统计
- ✅ 域名级可用率 = 该域名下**所有历史IP**（含 DNS 轮换已移除的旧IP）的成功次数之和 ÷ 总检测次数之和，完整反映域名真实历史
- ✅ IP 级可用率独立统计，每个 IP 单独记录
- ✅ 健康状态颜色区分：
  - 🟢 绿色：可用率 >80%，或全部 IP 在线
  - 🟡 黄色：可用率 50～80%，或有 IP 在线同时有 IP 离线（亚健康，优先显示）
  - 🔴 红色：可用率 <50%，或所有 IP 全部离线
- ✅ 延迟进度条颜色：绿色 ≤500ms、黄色 500～1500ms、红色 >1500ms 或超时

### 仪表盘
- ✅ 6格动态统计卡，全面重构为对半分列布局：
  - **卡1 在线/总监控**：在线数 + 在线率%，底部按 IPv4/IPv6 细分在线数 vs 总数
  - **卡2 故障/未知**：故障数 + 未知数，底部 IPv4/IPv6 各自细分，暂停数右列显示
  - **卡3 平均延迟**：总体 / IPv4 / IPv6 / TCP / UDP 五行分开显示，只统计在线IP（排除离线/超时/暂停）
  - **卡4 网络健康**：✔/✗ 居中显示，**鼠标悬停弹出探针详情浮层**（8.8.8.8 / 1.1.1.1 / 114.114.114.114 各自颜色标记可达/超时）
  - **卡5 TCP http/https**：在线 / 总数 / 可用率% / 故障数 / 未知数
  - **卡6 UDP**：在线 / 总数 / 可用率% / 故障数 / 未知数
- ✅ 可用率排行 TOP10（最差/最佳一键切换），固定高度精确容纳10行
- ✅ 告警中心：连续检测失败 ≥5 次自动触发，显示 IP 归属地、运营商、当前状态
- ✅ 错误日志聚合展示：相同错误去重，显示首次时间、出现次数、IP 归属地
- ✅ 快速搜索（全局）：默认展示所有监控项，支持翻页（10条/页），点击域名可筛选查看该域名所有IP详情
- ✅ 快速搜索支持：域名 / IP / 运营商 / 国家 / 状态 / 协议（http/https/udp）
- ✅ 快速搜索状态列**三态排序**（循环切换，无还原按钮）：
  - `▼` 离线优先（默认）：离线 → 在线 → 已暂停 → 未知
  - `?` 未知优先：未知 → 已暂停 → 离线 → 在线
  - `▲` 在线优先：在线 → 已暂停 → 未知 → 离线

### 代理支持
- ✅ HTTP/HTTPS Tracker 支持 HTTP CONNECT 代理
- ✅ UDP Tracker 支持 SOCKS5 代理（手动实现 RFC 1928，不依赖 PySocks）
- ✅ SOCKS5 长连接复用：TCP 控制连接全局保持，所有线程共享同一固定 UDP 源端口
- ✅ 固定源端口解决 SOCKS5 relay source port filter 丢包问题
- ✅ transaction_id 多路复用，并发线程互不干扰
- ✅ 代理健康状态机：连续失败 2 次进入 30s 冷却，冷却期内探测跳过不计入统计
- ✅ 代理不可用时保留上次检测状态，**不污染可用率数据**
- ✅ 代理地址智能双栈：IPv6 优先，IPv6 不可用自动 fallback 到 IPv4
- ✅ `localhost` 双栈歧义自动处理（系统默认解析到 `::1`，代理只监听 IPv4 时自动切换）
- ✅ SO_KEEPALIVE 保活，防止 NAT/防火墙静默断开长连接
- ✅ TCP 控制连接断开时自动重建，后台线程监控连接存活

### DNS 解析
- ✅ 三种 DNS 模式：系统 DNS / dnspython / 自定义 DNS 服务器
- ✅ 支持读取本地 hosts 文件（getaddrinfo 原生行为）
- ✅ 每轮检测前重新解析，自动感知 IP 变化
- ✅ DNS 解析失败时保留缓存 IP 继续检测，不中断监控
- ✅ **TCP 53 强制模式**：自定义/dnspython 模式下可开启 `dns_use_tcp`，所有 DNS 查询走 TCP 53 端口而非 UDP，解决国内 UDP DNS 高丢包问题
- ✅ 自定义 DNS 支持单台服务器独立指定 TCP：`tcp://8.8.8.8` 格式，与普通 IP 混用
- ✅ DNS 失败日志去重：同一域名解析失败只在首次输出控制台/Web 日志，恢复后重置，避免刷屏

### 监控列表
- ✅ 卡片式布局，展示端口、协议（TCP/UDP）、IP数量、添加时间
- ✅ 展开行显示每个 IP 的状态、可用率、延迟、归属地、运营商、最后检查时间
- ✅ IP 地址等宽字体显示，IPv4 绿色、IPv6 紫蓝色，附 v4/v6 角标
- ✅ TCP 协议自动标注 HTTP / HTTPS（端口443 = HTTPS）
- ✅ IP 列头三态点击：展开全部 → 折叠全部 → 还原默认（≤5 IP 自动展开）
- ✅ 历史IP（已移除）半透明灰色显示，可在配置中关闭
- ✅ 工具栏筛选：ALL / IPv4 / IPv6 · ALL / TCP / UDP / HTTP / HTTPS · 当前状态 · 关键词搜索
- ✅ 排序：可用率 / 域名 / 延迟 / 添加时间 / 最后检查 / 端口，支持升序/降序/还原三态
- ✅ 状态/可用率列头显示当前统计周期（24h / 7d / 30d）
- ✅ 延迟可视化进度条（颜色随延迟动态变化）
- ✅ 离线 IP 显示重试按钮（精确到 IP 级别，立即执行，不触发轮询等待）
- ✅ 批量添加 Tracker（多行粘贴，`|` 开头自动跳过）
- ✅ 暂停/恢复监控（支持单IP、单域名、全部）

### 可用率排行榜
- ✅ 域名显示 TCP/UDP 协议标签 + v4/v6 版本角标
- ✅ 显示活跃IP数及在线数
- ✅ 已暂停的域名自动从排行榜隐藏
- ✅ 最低可用率筛选（50% / 80% / 90% / 100%）
- ✅ 协议筛选：ALL / TCP / UDP
- ✅ IP版本筛选：ALL / IPv4 / IPv6
- ✅ 一键导出（**所见即所导**：当前筛选结果直接下载，已暂停域名不包含在内）
- ✅ URL 路由：`/ranking`

### Tracker 导出 API
- ✅ 公开 API，无需登录，任何人均可 GET 请求获取在线 Tracker 列表（纯文本，每行一个 URL）
- ✅ 旧路径兼容：`/trackers`、`/tracker.txt` 自动转发到 `/api/tracker`，无需客户端处理跳转
- ✅ 已暂停域名不包含在导出结果中
- ✅ 域名之间自动添加空行，兼容需要空行分隔的 BT 客户端
- ✅ 参数设计：

| 参数 | 可选值 | 默认 | 说明 |
|------|--------|------|------|
| `day` | `24h` `7d` `30d` | `24h` | 按哪个周期可用率排序 |
| `uptime` | `0` `50` `80` `90` `100` | `0` | 最低可用率（0=不过滤）|
| `net` | `all` `tcp` `udp` | `all` | 协议过滤 |
| `ip` | `all` `ipv4` `ipv6` | `all` | IP 版本过滤 |
| `url` | 任意字符串 | `/announce` | 追加到每行 URL 末尾的后缀 |

```bash
# 全量，无过滤
curl http://host/api/tracker

# 7天可用率 ≥ 90%，全协议，全 IP
curl "http://host/api/tracker?day=7d&uptime=90"

# 30天可用率 ≥ 90%，仅 TCP，仅 IPv4
curl "http://host/api/tracker?day=30d&uptime=90&net=tcp&ip=ipv4"

# 旧路径兼容（自动转发，无需 -L）
curl http://host/tracker.txt
```

### 单域名/IP 查询 API（新增）

对外开放的轻量查询接口，无需登录，可查询单个域名或 IP 的当前状态、可用率、延迟等信息。

- ✅ 支持域名或 IP 直接查询
- ✅ 返回格式：纯文本（默认）或 JSON
- ✅ 可自定义返回字段：`status` / `uptime` / `delay` / `location` / `checked`
- ✅ 频率限制：同 IP 每分钟 66 次

| 参数 | 可选值 | 默认 | 说明 |
|------|--------|------|------|
| `host` | 域名或IP | 必填 | 查询目标 |
| `list` | `status,uptime,delay,location,checked` | `status,uptime,delay,checked` | 返回字段（逗号分隔） |
| `type` | `txt` `json` | 仅带 host 时为 `txt`，带 list 时为 `json` | 返回格式 |

```bash
# 最简查询（纯文本）
curl "http://host/api/query?host=bt.rer.lol"
# → bt.rer.lol  Online  100.0%  1ms  2026-03-11T21:18:41.812562

# 查询 IPv6 地址
curl "http://host/api/query?host=2a06:1280:f115::2"
# → 2a06:1280:f115::2  Online  100.0%  310ms  2026-03-11T21:20:17.505961

# 指定字段 + 文本格式
curl "http://host/api/query?host=bt.rer.lol&list=status,uptime,delay,location,checked&type=txt"
# → bt.rer.lol  Online  100.0%  0ms  China · China Unicom Network  2026-03-11T20:46:18.974702

# JSON 格式
curl "http://host/api/query?host=bt.rer.lol&type=json"
# → {"host":"bt.rer.lol","status":"Online","uptime":"100.0%","delay":"1ms","checked":"..."}
```

### 网络健康检测
- ✅ 多目标探针（8.8.8.8 / 1.1.1.1 / 114.114.114.114 :53），**全部目标同时探测**（不提前中止），每个独立记录结果
- ✅ 任一可达即视为网络正常；全部不可达才告警
- ✅ 双重保障：探针状态 + 本轮失败率（≥90% 失败视为本地网络异常）
- ✅ 网络异常时本轮历史数据不计入统计，避免全局性误报污染数据
- ✅ 仪表盘卡片鼠标悬停，实时展示每个探针IP的最后一次探测结果（绿色=可达，红色=超时）

### IP 归属地
- ✅ 自动识别 IP 归属国家、运营商（ip-api.com，本地缓存）
- ✅ 启动后后台自动补查遗留的未知归属地（延迟10s开始，限速0.5s/IP）
- ✅ 每轮检测后若归属地仍为 Unknown，自动触发补查，不阻塞检测线程

### 界面与交互
- ✅ 日间/夜间主题切换，IPv4/IPv6 颜色在两种主题下均清晰可读
- ✅ 页面视野（50%～100%）可在导航栏实时调整，记录到 Cookie
- ✅ 多国语言：中文 / English / Русский / Français / 日本語 / 한국어（含 API 说明全文翻译）
- ✅ URL 路由（`/home`、`/trackers`、`/ranking`、`/logs`、`/config`）
- ✅ 所有搜索框支持 `ESC` 键一键清空
- ✅ 日志搜索：支持直接输入 `error` / `warn` / `info` 快速按级别过滤
- ✅ 操作反馈 Toast 通知
- ✅ gzip 压缩传输（HTML ~210KB → ~48KB）+ ETag 协商缓存（文件未变更返回 304）

### 账户与权限
- ✅ 三级角色权限：admin / operator / viewer
  - `admin`：完整权限，含配置页、用户管理、清空日志，重试不限速
  - `operator`：增删 Tracker、重试（500ms 冷却）
  - `viewer`：只读，重试（1s 冷却）
- ✅ 密码存储升级：PBKDF2-HMAC-SHA256 + 随机盐（200000轮），向后兼容旧版无盐 SHA256 格式自动迁移
- ✅ 登录后自动刷新配置，无需手动 F5
- ✅ 登录失败次数限制（10次锁定15分钟），防止暴力破解
- ✅ Session Cookie 签名（`session_secret.key`），重启后登录状态保持
- ✅ 未登录用户仅返回前端必要的公开配置字段，不暴露运维信息
- ✅ 管理员操作日志：暂停/恢复操作记录操作者用户名 + 脱敏 IP（Web界面显示 `1.*.*.4`，控制台保留完整 IP）
- ✅ 安全响应头：`X-Frame-Options` / `X-Content-Type-Options` / `Referrer-Policy` / `Content-Security-Policy`
- ✅ HSTS（`Strict-Transport-Security`）：仅在 `HTTPS_ENABLED=1` 环境变量下自动启用，内网 HTTP 部署不受影响
- ✅ `SESSION_COOKIE_SECURE` 自动检测：读取环境变量 `HTTPS_ENABLED=1` 自动启用，无需手动修改代码
- ✅ CSRF Token 双重验证（Header `X-CSRFToken` + Session 比对）

### 系统与运维
- ✅ waitress 生产服务器（消除 Flask 开发警告）
- ✅ 静态资源本地化（`static/` 目录），支持完全离线部署，无需 CDN
- ✅ 配置全部持久化到 `config.json`，重启自动读取
- ✅ 控制台日志分级：`none` / `info` / `error` / `debug`
- ✅ 日志按级别独立限制上限（Info / Success / Error 分别配置，互不影响）
- ✅ 配置变更日志只输出实际改变的字段（含可读标签和单位）
- ✅ 日志可选写入磁盘：`error.log`（错误日志）+ `access.log`（nginx 格式访问日志）
- ✅ nginx / Cloudflare 反向代理友好：`trust_cf_ip` 配置项控制是否信任 `CF-Connecting-IP` / `X-Forwarded-For` 获取真实客户端 IP
- ✅ 限流内存自动清理：后台线程定期清理 `_rate_limit_store` / `_login_fail` / `_retry_throttle` 等内存字典的过期条目，防止长期运行内存缓慢增长，清理间隔可配置（`cleanup_interval`）

---

## 🚀 快速部署

### 1. 安装依赖

```bash
pip install Flask flask-cors dnspython requests waitress
```

### 2. 文件结构

```
Network Monitor/
├── app.py
├── index.html
├── static/               # 本地静态资源（JS/CSS/字体）
│   ├── vue.global.js
│   ├── vue-i18n.global.js
│   ├── axios.min.js
│   ├── chart.umd.min.js
│   ├── bootstrap.min.css
│   ├── bootstrap-icons.css
│   ├── bootstrap.bundle.min.js
│   └── fonts/
│       ├── bootstrap-icons.woff2
│       └── inter-*.woff2
├── config.json           # 自动生成，配置持久化
├── data.json             # 自动生成，监控数据（当前状态 + 摘要）
├── history.json          # 自动生成，历史可用率数据（时间戳格式，主要数据源）
│                         #   ⚠️ 勿直接用记事本手动编辑紧凑格式，易破坏 JSON 结构
│                         #   建议用 fix_history.py 工具格式化后再编辑
├── error.log             # 自动生成（启用日志存盘后）
├── access.log            # 自动生成（启用日志存盘后，nginx 格式）
└── session_secret.key    # 自动生成，⚠️ 勿上传至公开仓库
```

### 3. 运行

```bash
python app.py
```

访问：`http://localhost:443`

### 4. 创建 start.bat 快捷启动（Windows）

```batch
@echo off
cd /d "%~dp0"
python app.py
pause
```

### 5. HTTPS 支持

项目支持通过环境变量启用 HTTPS：

```bash
# Windows
set HTTPS_ENABLED=1 && python app.py

# Linux/macOS
export HTTPS_ENABLED=1 && python app.py
```

- 首次启用 HTTPS 时，系统会自动生成自签名证书（`cert.pem` 和 `key.pem`）
- 浏览器会显示安全警告，但数据传输仍然是加密的
- 可通过环境变量自定义证书文件路径：
  - `HTTPS_CERT`：证书文件路径（默认：`cert.pem`）
  - `HTTPS_KEY`：私钥文件路径（默认：`key.pem`）

---

## 🌐 网络监听设置

`listen_port`、`listen_ipv4`、`listen_ipv6` 三项修改后需**重启应用**才能生效。

### IPv4 / IPv6 监听模式

| 模式 | IPv4 绑定地址 | IPv6 绑定地址 | 适用场景 |
|------|-------------|-------------|---------|
| `global` | `0.0.0.0` | `[::]` | 监听所有网卡（默认，公网/内网均可访问） |
| `local` | `127.0.0.1` | `[::1]` | 仅本机访问（搭配反向代理使用） |
| `custom` | 填 `listen_ipv4_custom` | 填 `listen_ipv6_custom` | 指定某块网卡的 IP 地址 |
| `disabled` | 不监听 | 不监听 | 纯 IPv6 或纯 IPv4 部署时禁用另一协议栈 |

**典型配置示例：**

```json
// 标准公网部署（双栈全监听，默认）
{ "listen_ipv4": "global", "listen_ipv6": "global" }

// 仅本机 + nginx 反代
{ "listen_ipv4": "local", "listen_ipv6": "disabled" }

// 纯 IPv6 公网
{ "listen_ipv4": "disabled", "listen_ipv6": "global" }

// 指定内网网卡
{ "listen_ipv4": "custom", "listen_ipv4_custom": "192.168.1.10",
  "listen_ipv6": "disabled" }
```

> ⚠️ `global` 模式下服务直接暴露在所有网卡，建议配合防火墙或 `trust_cf_ip` 使用。

---

## ⚠️ 安全注意事项

### `session_secret.key` 文件

此文件是 Flask Session Cookie 的签名密钥，**绝对不能上传到公开 Git 仓库**。一旦泄露，任何人都可以伪造 Cookie 绕过登录，直接获得 admin 权限。

**务必在 `.gitignore` 中添加：**

```gitignore
session_secret.key
config.json
data.json
history.json
error.log
access.log
```

**如何重置密钥：**

```bash
# 方法1：删除文件，下次启动自动重新生成
del session_secret.key        # Windows
rm session_secret.key         # Linux/macOS

# 方法2：手动生成新密钥
python -c "import os; open('session_secret.key','wb').write(os.urandom(32))"
```

重置密钥后，所有已登录用户的 Session 会立即失效，需要重新登录。

### 默认账户密码

首次运行后请立即修改默认密码（配置页 → 用户管理）：

| 账户 | 默认密码 | 权限 |
|------|----------|------|
| admin | admin | 完整权限 |
| operator | operator | 增删Tracker |
| viewer | viewer | 只读 |

---

## ⚙️ 配置参数说明

所有配置通过网页配置页面修改，**自动持久化**到 `config.json`，重启后自动读取。

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `listen_port` | `443` | Web 服务端口（443 需要管理员权限） |
| `check_interval` | `30` | 监控间隔（秒） |
| `timeout` | `5` | TCP/UDP 连接超时（秒） |
| `retry_mode` | `polling` | 重试模式：`polling`（5→15→30→60s）或固定秒数 |
| `retry_interval` | `5` | 固定重试间隔（`retry_mode` 非 polling 时生效） |
| `monitor_workers` | `120` | 并发检测线程数（建议 30～200） |
| `stagger_batch_proxy` | `5` | 代理模式每批发包数（错峰控流） |
| `stagger_delay_proxy` | `150` | 代理模式批间延迟（毫秒） |
| `stagger_batch_direct` | `5` | 直连模式每批发包数 |
| `stagger_delay_direct` | `100` | 直连模式批间延迟（毫秒） |
| `dns_mode` | `system` | DNS 解析模式：`system` / `dnspython` / `custom` |
| `dns_custom` | 空 | 自定义 DNS 服务器（`custom` 模式，多个用逗号分隔；支持 `tcp://8.8.8.8` 格式单独指定 TCP） |
| `dns_use_tcp` | `false` | 强制 DNS 查询走 TCP 53（`dnspython`/`custom` 模式有效；系统 DNS 模式下此项无效） |
| `listen_ipv4` | `global` | IPv4 监听模式：`global`（0.0.0.0）/ `local`（127.0.0.1）/ `custom`（自定义地址）/ `disabled` |
| `listen_ipv4_custom` | 空 | `listen_ipv4=custom` 时的自定义 IPv4 监听地址 |
| `listen_ipv6` | `global` | IPv6 监听模式：`global`（[::]）/ `local`（[::1]）/ `custom`（自定义地址）/ `disabled` |
| `listen_ipv6_custom` | 空 | `listen_ipv6=custom` 时的自定义 IPv6 监听地址 |
| `refresh_geo_on_restart` | `true` | 重启后是否自动在后台补查 IP 归属地（延迟10s开始，限速0.5s/IP） |
| `tracker_stat_period` | `24h` | 监控列表可用率统计周期：`24h` / `7d` / `30d` |
| `dashboard_stat_period` | `24h` | 仪表盘排行榜统计周期（进入后可手动切换） |
| `cache_history` | `true` | 可用率历史缓存持久化（写入 `history.json`） |
| `show_removed_ips` | `true` | 是否显示历史IP（已从DNS消失的IP，半透明灰色） |
| `default_layout_width` | `1700` | 默认页面视野宽度（px），Cookie 优先级更高 |
| `export_suffix` | `/announce` | 导出 Tracker 列表时追加的路径后缀 |
| `log_level` | `info` | 控制台日志级别：`none` / `info` / `error` / `debug` |
| `log_to_disk` | `false` | 日志是否写入磁盘（`error.log` + `access.log`） |
| `max_log_info` | `1000` | Info 级日志最大条目数 |
| `max_log_success` | `1000` | Success 级日志最大条目数 |
| `max_log_error` | `1000` | Error 级日志最大条目数 |
| `page_refresh_ms` | `30000` | 前端数据刷新间隔（毫秒），0 = 禁用 |
| `tab_switch_refresh` | `true` | 切换仪表盘/监控列表时是否自动刷新数据 |
| `http_proxy` | 空 | HTTP/TCP 代理地址 |
| `http_proxy_enabled` | `false` | 是否启用 HTTP 代理 |
| `udp_proxy` | 空 | UDP 代理地址（仅支持 SOCKS5） |
| `udp_proxy_enabled` | `false` | 是否启用 UDP 代理 |
| `allow_private_ips` | `false` | 是否允许添加内网IP（SSRF 防护，默认禁止） |
| `min_password_length` | `8` | 用户修改密码最小长度 |
| `cleanup_interval` | `3600` | 限流内存清理间隔（秒）。定期清理 `_rate_limit_store` / `_login_fail` / `_retry_throttle` 等内存字典的过期条目，防止长期运行内存增长。建议值 1800～7200 |
| `trust_cf_ip` | `false` | 是否信任 `CF-Connecting-IP` / `X-Forwarded-For` 获取真实客户端IP。通过 Cloudflare 部署时设为 `true`；直接暴露公网时保持 `false`（防IP伪造）；内网 HTTP 测试无需修改 |

---

## 📡 添加 Tracker

支持以下格式（可批量粘贴多行）：

```
udp://open.stealth.si:80/announce
udp://tracker.opentrackr.org:1337/announce
http://tracker.mywaifu.best:6969/announce
https://shahidrazi.online:443/announce
1.2.3.4:6969
| 这行以|开头，会被自动跳过（注释用）
```

- 多行粘贴，每行一个 Tracker
- `|` 开头的行自动跳过
- 支持 `IP:端口` 直接添加
- 空行自动忽略

---

## 🔒 代理配置

### UDP 代理（SOCKS5）

UDP Tracker 探测通过 SOCKS5 UDP Associate 转发，完整实现 RFC 1928，不依赖第三方库。

```json
{
  "udp_proxy": "socks5://127.0.0.1:1080",
  "udp_proxy_enabled": true
}
```

**技术原理：** 程序与 SOCKS5 代理建立一条长连接（TCP 控制连接），通过 UDP Associate 握手获取中继地址，之后所有检测线程共享同一 UDP socket（固定源端口），用 `transaction_id` 区分各线程收发包，互不干扰。

**注意事项：**
- 代理服务器必须支持 UDP Associate（CMD=0x03）
- 支持 `localhost`、IPv4、IPv6（`[::1]:port` 格式）
- `localhost` 默认优先尝试 IPv6（`::1`），若代理只监听 IPv4 自动 fallback 到 `127.0.0.1`
- 代理不可用时，UDP 检测自动跳过并保留上次状态，不影响可用率统计

### HTTP 代理

```json
{
  "http_proxy": "http://127.0.0.1:7890",
  "http_proxy_enabled": true
}
```

---

## ❌ 离线原因说明

| 显示 | 含义 |
|------|------|
| `超时(>Xs)` | 目标在超时时间内无响应 |
| `端口未开放` | 目标主动拒绝（ICMP Port Unreachable / TCP RST） |
| `代理连接失败` | SOCKS5 代理本身无法连接或握手失败 |
| `代理不可用(Xs后重试)` | 代理进入冷却期，当前跳过检测，保留上次状态 |
| `无效响应` | 收到回包但格式不符合协议 |

---

## ❓ 常见问题

**1. 无法启动？**
```bash
pip install Flask flask-cors dnspython requests waitress
```

**2. `index.html` 找不到？**
确保 `index.html` 与 `app.py` 在同一目录，或放在 `templates/` 子目录。

**3. 端口被占用？**
修改 `config.json` 中的 `port` 字段，或修改 `app.py` 中 `DEFAULT_CONFIG['port']`。

**4. 排行榜为空？**
排行榜需要积累历史记录，刚添加的 Tracker 需等待几轮检查后才会出现。开启「缓存统计可用率」可在重启后保留历史数据。

**5. 部分IP归属地显示为空？**
重启 app 后会自动启动后台补查线程（延迟10s开始），逐个查询未知归属地，无需手动干预。可用以下命令测试 ip-api.com 连通性：
```bash
curl -s "http://ip-api.com/json/1.2.3.4?fields=country,countryCode,isp"
```

**6. 只解析到部分 IP？**
确保网络支持 IPv6，或切换 DNS 模式为 `dnspython` 或 `custom`。程序同时查询 A 记录（IPv4）和 AAAA 记录（IPv6）。

**7. UDP 代理全部超时？**
确认代理软件已开启 UDP 转发功能。部分代理默认仅支持 TCP，需要在配置中单独开启 UDP Associate。

**8. 代理不可用时大量显示离线？**
代理连续失败 2 次后进入 30s 冷却期，冷却期内所有 UDP 探测自动跳过并保留上次状态，不产生误报。

**9. 多人使用时如何分配权限？**
在「配置 → 用户管理」中添加账户并分配角色：admin 完整权限 / operator 可增删Tracker / viewer 只读。

**10. 使用 `localhost` 代理但连接失败？**
程序优先尝试 IPv6（`::1`），若代理只监听 IPv4 会自动 fallback 到 `127.0.0.1`。若仍失败，可直接填写 `socks5://127.0.0.1:端口`。

**11. API 导出结果为空？**
已暂停的域名不会出现在 API 导出结果中。请检查目标域名是否处于暂停状态。

**12. `/trackers` 或 `/tracker.txt` 请求返回跳转？**
当前版本已改为直接返回内容，无需 `-L` 参数。如仍遇到问题，请确认使用的是最新版 `app.py`。

**13. 重启后域名可用率数据是否准确？**
是。历史数据以时间戳格式存储在 `history.json`，重启后直接按时间窗口切割计算，不依赖固定轮次计数，24H/7D/30D 数据均完整保留。DNS 轮换导致的已移除旧IP数据也会保留并纳入域名级统计，不会因IP更换而丢失历史故障记录。

**14. history.json 会无限增大吗？**
不会。程序每小时自动 GC（垃圾回收），清除30天前的记录。手动删除域名时，该域名的全部历史数据也会同步清除。

**15. 通过 Cloudflare 部署时如何获取真实客户端 IP？**
在配置页面将 `trust_cf_ip` 设为 `true`（或直接修改 `config.json`）。启用后程序优先读取 `CF-Connecting-IP` 头，CF 会强制覆盖此头为真实客户端IP，不可伪造。直接暴露公网时请保持 `false`，否则攻击者可伪造头部绕过限流。

**16. 如何在 HTTPS / Cloudflare 后面部署时启用 Secure Cookie？**
在系统环境变量中设置 `HTTPS_ENABLED=1`，程序启动时会自动将 `SESSION_COOKIE_SECURE` 设为 `True` 并添加 HSTS 响应头。内网 HTTP 测试时不设置此变量即可，无需修改代码。

**17. 登录失败多少次会被锁定？**
连续登录失败 10 次后，该 IP 将被锁定 15 分钟，期间所有登录请求直接返回 429。锁定期满后自动解除，或重启程序也会重置计数。

**18. DNS 解析一直失败，控制台刷屏怎么办？**
程序已内置日志去重机制：同一域名 DNS 失败只在**首次**输出控制台和 Web 日志，后续轮次静默跳过，不再重复打印。监控和 Web 端的 `DNS ERR` 红色标记仍会正常显示。待该域名 DNS 恢复后，静默标记自动清除，下次再失败时会重新提醒。若需测试 DNS 是否真正可达：
```bash
# 测试 UDP（默认）
nslookup retracker.lanta.me 8.8.8.8

# 测试 TCP 53（开启 dns_use_tcp 后对应的行为）
nslookup retracker.lanta.me 8.8.8.8 -vc
```

**19. 在中国大陆使用时 UDP DNS 丢包率高如何解决？**
在配置页 → DNS 设置中：
1. 将「解析模式」改为 **自定义**
2. 填入 DNS 服务器，如 `8.8.8.8,8.8.4.4`
3. 勾选「**强制使用 TCP 53 查询**」

TCP 53 连接可靠性远高于 UDP，国内网络环境下 GFW 对 UDP DNS 的干扰较多，改为 TCP 后通常可以解决解析失败问题。也可以只对单台服务器指定 TCP，格式：`tcp://8.8.8.8`（关闭全局开关时生效）。

**20. 使用 Cloudflare 的安全 DNS（如 `2606:4700:4700::1113`）导致大量 tracker 解析成 `[::]` 或 `127.0.0.1` 怎么办？**

这是 Cloudflare 安全 DNS 的 NXDOMAIN 拦截行为：当 tracker 域名被判定为恶意或不存在时，DNS 会返回 `[::]`（IPv6 空地址）或 `127.0.0.1` 作为替代地址，导致监控显示全部在线（实际无法连接）或全部离线，可用率数据被严重污染。

**解决方案：**
1. **切换 DNS 服务器**：在配置页 → DNS 设置中改用普通 DNS，如 `8.8.8.8` 或 `1.1.1.1`（非安全版）
2. **清理已污染的历史数据**：程序启动时会自动过滤 `[::]`、`127.0.0.1`、`0.0.0.0`、`::1` 等无效 IP 的历史记录，无需手动操作

**21. 手动编辑 `history.json` 后启动报错 `Expecting ',' delimiter: line 1 column XXXXXXX`，数据全丢了怎么办？**

这是 JSON 格式破坏导致的解析失败。`history.json` 默认以紧凑格式（无换行、无缩进）存储，文件体积可达数 MB 甚至数十 MB，在记事本等编辑器中**极难定位和手动修改**，一个多余或缺少的逗号就会导致整个文件解析失败，程序保存时会用 `{}` 覆盖损坏的文件，造成数据全丢。

**正确做法——使用 `fix_history.py` 工具：**

```bash
# 将 fix_history.py 放到与 history.json 同一目录，然后运行：
python fix_history.py
```

该工具会：
- 验证 JSON 合法性（明确告知错误位置）
- 自动过滤 `[::]`、`127.0.0.1` 等无效 IP
- 将文件重新格式化为**每个 IP 单独一行**，方便阅读和编辑
- **原子写入**：先写临时文件再替换，过程中不会损坏原文件
- 自动备份原文件为 `history.json.bak`

格式化后的文件结构示例：
```json
{
  "tracker.example.com": {
    "ip:1.2.3.4": [[1700000000,1],[1700000030,0]],
    "ip:5.6.7.8": [[1700000000,1]]
  },
  "other.tracker.org": {
    "ip:2a06:1280::1": [[1700000000,1]]
  }
}
```
删除某个 IP 时，整行删除即可；注意最后一个 IP 行末**不能有逗号**。

**22. 为什么现在 `app.py` 保存 `history.json` 不会再出现数据全丢的问题？**

新版本 `save()` 方法改为**原子写入**：
1. 先将数据写入临时文件 `history.json.tmp`
2. 写入完成并验证后，用 `os.replace()` 原子替换正式文件
3. 若写入过程中程序崩溃或系统异常，原 `history.json` 完全不受影响，临时文件会在下次启动时自动清理

`os.replace()` 在同一文件系统下是原子操作（POSIX `rename` 语义），不存在"写了一半"的中间状态。

---

## 🛠️ 技术栈

| 组件 | 说明 |
|------|------|
| 后端 | Python 3.10+ / Flask + waitress |
| 前端 | Vue 3 + Bootstrap 5 + vue-i18n |
| DNS | dnspython（IPv4+IPv6 双栈） |
| IP归属地 | ip-api.com 免费 API（本地缓存，启动后自动补查未知IP） |
| 代理 | 手动实现 SOCKS5 UDP Associate（RFC 1928），固定源端口 + tid 多路复用 |
| 数据持久化 | `config.json`（配置）+ `data.json`（当前状态摘要）+ `history.json`（时间戳历史库） |
| 认证 | Flask Session（Cookie 签名，PBKDF2+盐密码存储，`session_secret.key` 本地持久化） |
| 安全 | CSP / HSTS / `SESSION_COOKIE_SECURE` 自动检测、CSRF 双重验证、登录锁定、`trust_cf_ip` CF感知IP解析 |
| 压缩/缓存 | gzip 响应压缩 + ETag 协商缓存（304 Not Modified） |
| 访问日志 | nginx 格式 `access.log`（启用存盘时）+ 控制台分级输出 |

---

## 📄 许可证

MIT License

---

##### 项目发起人：rer
##### 项目协作者：minimaxi、Claude、豆包、grok、deepseek、Trae(solo)、Gemini、Cursor