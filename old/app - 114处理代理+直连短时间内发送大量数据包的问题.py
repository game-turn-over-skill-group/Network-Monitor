...
def monitor_loop():
    ...
    while True:
        try:
            ...
            # 代理模式：每批8个，间隔120ms —— 配合代理"每5包换端口"的轮换节奏
            # 直连模式：每批20个，间隔50ms —— 只打散突发，不影响整体速度
            use_proxy_stagger = bool(CONFIG.get('proxy_enabled') and CONFIG.get('udp_proxy','').strip())
            if use_proxy_stagger:
                STAGGER_BATCH = 8     # 与代理 udp-rotate 包数匹配，留出端口轮换时间
                STAGGER_DELAY = 0.12  # 120ms：代理端口切换需要 ~100ms
            else:
                STAGGER_BATCH = 20    # 直连：20个并发足够打散NAT突发
                STAGGER_DELAY = 0.05  # 50ms：只是错峰，不需要等端口轮换
