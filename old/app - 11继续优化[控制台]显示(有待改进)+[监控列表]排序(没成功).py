    ...
    try:
        if os.path.exists(CONFIG_FILE):
            ...
            for k in ['check_interval','timeout','retry_mode','retry_interval',
                      ...
                      'dns_mode','dns_custom','max_log_entries']:
                if k in saved:
                    cfg[k] = saved[k]
    except Exception:
        pass
...
def monitor_loop():
    ...
    cprint("监控服务启动", 'info')
    # 并发工作线程数：每个 IP 独立线程，最多 MONITOR_WORKERS 个并发
    MONITOR_WORKERS = 32
    while True:
        try:
            ...
            # ── 第一步：并行重新解析所有域名 DNS ─────────────────────────
            with ThreadPoolExecutor(max_workers=16) as dns_pool:
                dns_futures = {
            ...
            s = db.get_stats()
            summary = f"轮检完成 | 共{len(tasks)}个IP | 在线:{s['alive']} v4:{s['ipv4']} v6:{s['ipv6']}"
...
def log_response(response):
    if request.path.startswith('/api/') and request.method in ('POST','DELETE','PUT'):
        cprint(f"→ {response.status_code} {request.method} {request.path}", 'info')
    return response
...
def api_nav():
    """前端切换页面时调用，让控制台显示访问记录"""
    data   = request.json or {}
    tab    = data.get('tab', '?')
    remote = request.remote_addr
    # X-Forwarded-For 支持反向代理场景
