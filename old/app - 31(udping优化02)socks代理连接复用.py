    ...
    def get(self, proxy_url: str, timeout: int) -> 'Socks5UdpSocket':
        ...
        with self._lock:
            ...
            if not self._sock or self._sock._closed or self._sock._tcp_ctrl is None:
                self._build_locked(proxy_url, timeout)

            return self._sock

    def invalidate(self):
        """主动标记连接失效（发包/收包出错时调用，下次 get 会重建）"""
        with self._lock:
            if self._sock:
                self._sock._closed = True   # 让监控线程感知到
    ...
    if udp_proxy and 'socks5' in udp_proxy.lower():
        for attempt in range(2):   # 失败时重建连接重试一次
            ...
            try:
                ...
                # 连接可能已断，标记失效，下次 get 会重建
                _socks5_pool.invalidate()
                if attempt == 0:
                    cprint(f'[SOCKS5Pool] 连接异常，重建后重试: {e}', 'debug')
                    # 重新生成 packet 和 tid（重试用新的 transaction_id）
                    packet = _udp_tracker_packet()
                    tid    = packet[12:16]
                    continue
