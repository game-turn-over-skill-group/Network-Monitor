...
def cprint(msg: str, level: str = 'info', raw: bool = False):
    ...
    raw=True：原样输出（nginx access log 格式，不加时间前缀）
    raw=False：加 HH:MM:SS [LEVEL] 前缀（内部日志）
    """
...
def security_headers(response):
    ...
    try:
        size = len(response.get_data(as_text=False))
    except Exception:
...
def api_check():
    ...
    for ipi in ips_snap:
        ...
        if status == 'skipped':
            ...
            summary  = f"{ipi['ip']} → {status} {lat_s}"
        db.add_log(res_msg, 'info')
        result_summaries.append(summary)
        results.append({'ip':ipi['ip'],'status':status,'latency':lat,'error':err})

    # 控制台：一行结果摘要（内部格式）+ after_request 自动输出 nginx 行 = 共2行
    summary_str = ' | '.join(result_summaries) if result_summaries else '无IP'
    cprint(f"重试 {protocol.upper()}://{domain}:{port}{tag}: {summary_str}", 'info')
    g.access_note = f"retry {protocol.upper()}://{domain}:{port}{tag} → {summary_str}"
