                ...
                for d, t in self.trackers.items():
                    ...
                    for ip_obj in t['ips']:
                        ip_entry = {k: v for k, v in ip_obj.items()
                                    if k not in ('history_24h','history_7d','history_30d')}
                        if cache_hist:
                            # 24h 存原始数组（最多2880条），保持时间顺序，重启后准确区分各周期
                            ip_entry['history_24h'] = ip_obj.get('history_24h', [])
                            # 7d/30d 存压缩计数器（节省磁盘），重启后按各自上限还原
