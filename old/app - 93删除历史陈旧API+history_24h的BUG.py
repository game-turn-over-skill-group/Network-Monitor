        ...
        with self.lock:
            ...
            for domain, t in self.trackers.items():
                ...
                for ip_obj in t.get('ips', []):
                    ...
                    # 同时附加三个周期（前端可按需使用）
                    # IP级uptime_24h：用IP自己的24h历史（最精确）
                    ph24 = ip_obj.get('history_24h', [])
                    ip_copy['uptime_24h'] = round(sum(ph24)/len(ph24)*100, 1) if ph24 else None
                    # IP级uptime_7d/30d：用域名级历史（跨重启积累更准确，避免重启后三周期完全相同）
                    for pk in ('7d','30d'):
                        dh = t.get(f'history_{pk}', [])
                        ip_copy[f'uptime_{pk}'] = round(sum(dh)/len(dh)*100, 1) if dh else None
                ...
                for d, t in self.trackers.items():
                    ...
                    for ip_obj in t['ips']:
                        ip_entry = {k: v for k, v in ip_obj.items()
                                    if k not in ('history_24h','history_7d','history_30d')}
                        if cache_hist:
                            # IP级只存24h历史（用于IP独立可用率）；7d/30d统一用域名级历史
                    ...
                    if cache_hist:
                        # 域名级：24h 存原始数组，7d/30d 存压缩计数
                        entry['history_24h'] = t.get('history_24h', [])
